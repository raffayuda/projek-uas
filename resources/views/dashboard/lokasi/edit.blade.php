...
<div class="max-w-2xl mx-auto py-8 px-4">
    <div class="bg-white rounded-xl shadow-lg p-8">
        <div class="flex items-center justify-between mb-6">
            <h2 class="text-2xl font-bold text-gray-800">Edit Lokasi</h2>
            <a href="{{ route('lokasi.index') }}" class="text-gray-600 hover:text-gray-800 transition-colors">
                <i class="fas fa-times text-xl"></i>
            </a>
        </div>

        <form action="{{ route('lokasi.update', $lokasi->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="space-y-4">
                ...
                <div>
                    ...
                    <textarea name="alamat" id="alamat" rows="3" 
                              class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors"
                              placeholder="Masukkan alamat lengkap lokasi rental" required>{{ old('alamat', $lokasi->alamat) }}</textarea>
                    @error('alamat')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <input type="text" name="koordinat" id="koordinat" value="{{ old('koordinat', $lokasi->koordinat) }}" 
                           class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors"
                ...
                <div>
                    ...
                    @if($lokasi->image)
                    <div class="mb-4">
                        <img src="{{ asset('storage/location-images/' . $lokasi->image) }}" alt="{{ $lokasi->nama }}" class="w-32 h-32 object-cover rounded-lg shadow">
                    </div>
                    @endif
