...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
        ...
        <div class="bg-white rounded-3xl shadow-xl p-8 flex flex-col md:flex-row items-center gap-8 mb-12">
            <div class="flex-1 w-full max-w-md">
                <div class="text-2xl font-bold text-gray-900 mb-2" id="currentLocationName">
                    {{ $locations[0]->nama ?? 'No Location' }}
                </div>
                <div class="text-gray-500 mb-4" id="currentLocationAddress">
                    {{ $locations[0]->alamat ?? 'Address not specified' }}
                </div>
            ...
            <div class="hidden md:block flex-1 w-full">
                <img 
                    ...
                    alt="{{ $locations[0]->nama ?? 'Location' }} Image" 
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-8">
            ...
            <div class="bg-white rounded-2xl shadow-lg p-6 flex flex-col items-center text-center transition hover:shadow-2xl hover:-translate-y-1 duration-300 location-card">
                <img 
                    ...
                    alt="{{ $loc->nama ?? 'Location ' . ($index + 1) }} Image" 
                    class="w-28 h-28 object-cover rounded-xl mb-4 shadow-md"
                    onerror="this.src='https://images.unsplash.com/photo-1506744038136-46273834b3fb?auto=format&fit=crop&w=600&q=80'">
                <div class="font-semibold text-lg text-gray-900 mb-1">{{ $loc->nama ?? 'Location ' . ($index + 1) }}</div>
                <div class="text-gray-500 text-sm mb-2">{{ $loc->alamat ?? 'Address not specified' }}</div>
                ...
                <button 
                    ...
                    data-index="{{ $index }}"
                    data-nama="{{ e($loc->nama ?? 'Location ' . ($index + 1)) }}"
                    data-alamat="{{ e($loc->alamat ?? 'Address not specified') }}"
    ...
    <div class="bg-white rounded-3xl shadow-xl p-12 text-center">
        <h3 class="text-2xl font-bold text-gray-900 mb-4">No Locations Available</h3>
        <p class="text-gray-500 text-lg">We're currently updating our location information. Please check back later.</p>
    </div>
    @endif
</div>
